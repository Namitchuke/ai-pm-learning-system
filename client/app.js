...
${open ? `<div style="font-size:13px;color:var(--t3);margin-top:8px;padding-left:12px;border-left:3px solid var(--purple)">💡 <strong>Hint/Structure:</strong> ${ib.hint}</div>` : ''}
</div>`;
            });
            h += `</div>`;
            // Decision Logs
...
<div class="sm"><div class="sm-val">${calcOverallQuizPct()}%</div><div class="sm-lbl">Quiz Average</div></div>
        ...
        function renderTab3() {
            ...
            filtered.forEach((m, i) => {
                ...
                ${open ? `<div style="font-size:13px;color:var(--t2);margin-top:12px;padding-left:12px;border-left:3px solid var(--purple);background:rgba(255,255,255,0.03);padding-top:8px;padding-bottom:8px;border-radius:4px">💡 <strong>Suggested Flow / Hints:</strong><br>${m.flow}</div>` : ''}
                </div>`;
            });
            h += `</div>`;
            el.innerHTML = h;
        ...
        function openSkillQuiz(skillId, level) {
            ...
            let h = `<h4 style="font-size:12px;font-weight:700;color:var(--purple);margin-bottom:8px">Level ${level} Quiz</h4>`;
            let answered = 0, correct = 0;
